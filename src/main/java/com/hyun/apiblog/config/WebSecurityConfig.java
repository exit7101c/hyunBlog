package com.hyun.apiblog.config;


import com.hyun.apiblog.config.filter.CustomAuthenticationFilter;
import com.hyun.apiblog.config.handler.CustomAuthFailureHandler;
import com.hyun.apiblog.config.handler.CustomAuthSuccessHandler;
import com.hyun.apiblog.config.handler.CustomAuthenticationProvider;
 ...
 * 웹 서비스가 로드 될때 Spring Container 의해 관리가 되는 클래스이며 사용자에 대한 ‘인증’과 ‘인가’에 대한 구성을 Bean 메서드로 주입
 */
@Slf4j
@Configuration
@EnableWebSecurity
public class WebSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        // [STEP1] 서버에 인증정보를 저장하지 않기에 csrf를 사용 x
        http.csrf().disable();

        // [STEP2] form 기반의 로그인에 대해 비 활성화하며 커스텀으로 구성한 필터를 사용
    ...
    public CustomAuthenticationFilter customAuthenticationFilter() {
        ...
        customAuthenticationFilter.setAuthenticationSuccessHandler(customLoginSuccessHandler());    // '인증' 성공 시 해당 핸들러로 처리를 전가
        customAuthenticationFilter.setAuthenticationFailureHandler(customLoginFailureHandler());    // '인증' 실패 시 해당 핸들러로 처리를 전가
